...
public class Car {
	...
	public void expandCar() {
		Door[] door2=new Door[door.length+2];	//new a array door2 that is longer the door
		for(int i=0;i<door2.length;i++) {
			if(i<door.length) {
				door2[i]=door[i];	//copy the element in door to the door2
			}
			door2[i]=new Door();	//add 2 new element
		}
		door=door2;		
	...
	public static void testCar() throws Exception {
		try{
			...
			System.out.println(c2.door[2].isOpen()==false);
			System.out.println(c2.door[3].isOpen()==false);
			System.out.println(c2.door[4].isOpen()==false);
			System.out.println(c2.door[5].isOpen()==false);	
